...
void Button::onSelfRender(int renderX, int renderY) {
	...
	if (isChecking()) {
		engine.drawRestart();
		if (isHolding())
			engine.drawRect(renderX, renderY, w, h, 0x28000000, 0x28000000, 0x28000000, 0x28000000);
		else engine.drawRect(renderX, renderY, w, h, 0x28ffffff, 0x28ffffff, 0x28ffffff, 0x28ffffff);
	}
